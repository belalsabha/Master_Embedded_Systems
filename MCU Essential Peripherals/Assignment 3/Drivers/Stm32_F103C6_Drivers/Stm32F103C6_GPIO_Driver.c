...
void MCAL_GPIO_Init(GPIOx_t *GPIOx , GPIO_Configure_Pin_t* PinNumber_Configuration ){
   ...
   (*Configure_Register) &= ~(Get_Position_in_CRL_CRH(PinNumber_Configuration->GPOI_Pin_Number));

   //if Pin is output
   if(PinNumber_Configuration->GPIO_Mode == output_push_pull || output_Open_drain || Alternate_function_output_Push_pull || Alternate_function_output_Open_drain){

	   //Set  CNFy[1:0] MODEy[1:0]
	   PIN_Config = ( (((PinNumber_Configuration->GPIO_Mode - 4 ) << 2) | (PinNumber_Configuration->GPIP_Output_Frequency)) & 0x0f ) ;
   }//if pin is input
   else   //MODEy[1:0]
   {
	   if(PinNumber_Configuration->GPIO_Mode == GPIO_Analog_Mode  || GPIO_Floating_Input ){
		   ...
		   PIN_Config = (((PinNumber_Configuration->GPIO_Mode << 2) | 0x1 ) &0x0F) ;  //GPIO_Floating_Input
 ...
 * Note			-none
 */
uint8_t  MCAL_GPIO_ReadPin(GPIOx_t *GPIOx , uint8_t PinNumber){
 ...
 * Note					-none
 */
void MCAL_GPIO_WritePin(GPIOx_t *GPIOx , uint8_t PinNumber , uint8_t Value){
 ...
 * Note					-none
 */
void MCAL_GPIO_WritePort(GPIOx_t *GPIOx , uint8_t Value){
 ...
 * Note				-	-none
 */
void MCAL_GPIO_TogglePin(GPIOx_t *GPIOx , uint8_t PinNumber){
	GPIOx->ODR ^= (PinNumber) ;
}
 ...
 * Note					-none
 */
uint8_t MCAL_GPIO_LockPin(GPIOx_t *GPIOx , uint8_t PinNumber){
...
Each lock bit freezes the corresponding 4 bits of the control register (CRL, CRH).*/
	...
	temp = GPIOA->LCKR; 							  // Read 0
	temp = GPIOA->LCKR; 							  // Read 1 (lock confirmed)
